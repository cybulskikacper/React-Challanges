...
export default function Article(props) {
	const extraFavClass = favoritedArticles.find(article => article.id === props.articleData.id) ? 'favorited' : undefined

	return (
		<div key={props.articleData.id} className="article-container">
			<h3>{props.articleData.title}</h3>

			<img src={props.articleData.image} alt={props.articleData.title} />

			<div className={`article-text-container ${props.articleData.expanded ? 'expanded' : 'minimized'}`}>
				<p>{props.articleData.text}</p>
			</div>

			<button className="read-more-button" data-button-type="toggleExpand" data-article-id={props.articleData.id}>
			...
			<div className="article-buttons-container">
				<button
					...
					data-article-id={props.articleData.id}></button>
