...
export default function App() {
	...
	const messageClass = answerStatus.toLowerCase().split(' ').join('-').slice(0, -1)

	const gamePlayButtons = [
		<GetNewProblemButton disabled={recentCorrectAnswer} clickHandler={getNewProblem} key={nanoid()} />,
	...
	function showStates() {
		...
		for (const state in states) {
			...
			const value =
				states[state] === '' ? `""` : typeof states[state] === 'object' ? stringify(states[state]) : states[state]
	...
	return (
		<div className="wrapper">
			{gameStarted ? null : <h1>Math-o-Matic</h1>}

			<form onSubmit={handleSubmit}>
				<label>
					{!gameStarted ? null : <div className="problem-container">{mathProblem.string}</div>}
				...
				{recentStatusChange && <div className={`message-container ${messageClass}`}>{answerStatus}</div>}

				<div className="button-container">{gameStarted ? gamePlayButtons : startButton}</div>
			</form>
		</div>
	)
}
