...
export default function App() {
	...
	const [prisonMusic, { pause: pausePrisonMusic }] = useSound('./src/assets/audio/music/PrisonMusic.mp3', {
		volume: 0.07,
	})

	const [freedomMusic, { pause: pauseFreedomMusic }] = useSound('./src/assets/audio/music/FreedomMusic.mp3', {
		volume: 0.1,
	})
	...
	function startGame(e, alt) {
		const allKeys = ['ArrowDown', 'ArrowUp', 'ArrowLeft', 'ArrowRight', 'b', 'a']

		const mouseEvent = e ? e.type === 'mousedown' || e.type === 'click' : undefined
	...
	function placeBomb(bomb, pookachu) {
		if (gameStarted && !bomb.planted && !escaped) {
			soundOn && boink()
			setBomb(prevBomb => {
				let xPosition = getBombPosition('x', pookachu.xPosition, pookachu.direction)
				let yPosition = getBombPosition('y', pookachu.yPosition, pookachu.direction)
				return {
					...prevBomb,
					planted: true,
					xPosition: xPosition,
	...
	function updateMovement(event) {
		...
		if (event.type === 'keydown' || event.type === 'keyup') {
			const choice = directionChoices.find(choice => choice.eventName === event.key)
			if (choice) {
				direction = choice.direction
			}
		} else if (event.type === 'mousedown' || event.type === 'mouseup' || event.type === 'mouseleave') {
	...
	return (
		<div className="wrapper" onKeyDown={gameStarted ? handleKey : startGame} onKeyUp={gameStarted ? handleKey : null}>
			<div className="console-container">
				<img className="pookachu-background-img" src="./src/assets/images/other/PookachuBackgroundImage.png" />
				<Controls />
				<GameScreen
				...
				<div className="overall-buttons-container">
					...
					<ActionButton
						...
						clickHandler={gameStarted ? () => placeBomb(bomb, pookachu) : startGame}
					/>
